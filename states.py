...
class EnterMineAndDigForNugget(State):
    ...
    def slowly_dying(sefl, miner):
       if miner.fatigue >=1:
            print ("Miner {}: Feeling tired... ARGH! I done slipped and fell on my pick-axe".format(miner.name))
            miner.life_points - 2
...
class GoHomeAndSleepTillRested(State):
    ...
    def slowly_dying(self, miner):
        if miner.thirst >= 1:
            print("Miner {}: I done got too thirsty in bed and my lungs are dried up".format(miner.name))
        miner.life_points - 2
...
class QuenchThirst(State):
    ...
    def slowly_dying(self, miner):
        if miner.fatigue >= 1:
            print("Miner {}: Now I done really messed up.. I got drunk and my head hit the toilet".format(miner.name))
        miner.life_points - 6
